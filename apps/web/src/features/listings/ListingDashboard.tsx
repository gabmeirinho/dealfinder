    ...
    <section className="listing-review" aria-labelledby="inbox-title">
      <header className="listing-heading">
        <div>
          ...
          <p>Review scored cars, keep your buying decisions durable, and prepare the next conversation without contacting a seller.</p>
        </div>
      ...
      <form className="listing-filters" onSubmit={(event) => {
        ...
        setAppliedFilters({ state, query: query.trim(), riskOnly, archived });
      }}>
  ...
  return (
    <li className={`listing-inbox-row ${active ? "is-selected" : ""} ${listing.risk?.reasons.length ? "is-risk" : ""}`}>
      <button type="button" onClick={onOpen} aria-label={`Review ${listing.title}`}>
        <span className="listing-score">{listing.score?.total ?? "—"}<small>score</small></span>
        <span className="listing-row-main"><strong>{identity || listing.title}</strong><span>{listing.displayedPrice ?? "Price unknown"} · {listing.location ?? "Location unknown"}</span><small>Seen {formatDate(listing.lastSeenAt)} · {listing.processing?.state ?? "not processed"}</small></span>
...
function ListingInspector({ listing, client, onChange, onClose, onError }: {
  ...
  return (
    <article className="listing-inspector" aria-labelledby="listing-detail-title">
      ...
      <section className="fact-comparison"><header><h3>Vehicle facts</h3><button className="text-action" type="button" onClick={() => setCorrectionOpen((open) => !open)}>{correctionOpen ? "Close correction" : "Correct a fact"}</button></header><FactTable original={listing.normalizedFacts} effective={listing.effectiveFacts} corrections={new Set(listing.corrections.map(({ field }) => field))} />{correctionOpen ? <CorrectionForm listing={listing} client={client} onChange={onChange} onError={onError} /> : null}</section>

      <section className="original-copy"><h3>Original listing text</h3><h4>{listing.original.title}</h4>{listing.original.description === null ? <p className="muted-copy">No description was captured from the result card.</p> : <p>{listing.original.description}</p>}<ul>{listing.original.cardFacts.map((fact, index) => <li key={`${fact}-${index}`}>{fact}</li>)}</ul>{listing.sourceUrl !== null ? <a href={listing.sourceUrl} target="_blank" rel="noopener noreferrer">Open source listing</a> : <p className="muted-copy">Source URL was blocked because it was not a safe Facebook HTTPS URL.</p>}</section>

      <ScoreSection listing={listing} />

      <section className="detail-grid"><div><h3>Price history</h3>{listing.priceHistory.length === 0 ? <p className="muted-copy">No parsed price history yet.</p> : <ol className="price-history">{listing.priceHistory.map((point) => <li key={point.id}><time>{formatDate(point.observedAt)}</time><strong>{formatEur(point.priceCents)}</strong>{point.previousPriceCents !== null ? <span>{priceDelta(point.priceCents, point.previousPriceCents)}</span> : null}</li>)}</ol>}</div><div><h3>Distance & matching</h3>{listing.matches.map((match) => <div className="match-record" key={match.searchId}><strong>{match.searchName}</strong><span>{match.distance?.distance.label ?? "Distance unknown"}</span><span>{match.evaluation?.eligible === true ? "Hard filters passed" : "Review filter mismatches"}</span></div>)}</div></section>
...
function ScoreSection({ listing }: { listing: ListingDetail }): ReactElement {
  const score = listing.scores[0];
  return <section className="score-detail"><h3>Deal score</h3>{score === undefined ? <p className="muted-copy">Scoring is pending or the listing does not belong to an eligible cohort.</p> : <><div className="score-summary"><strong>{score.score.total}<small>/100</small></strong><span>{score.score.confidence} confidence<br />{score.score.marketDataLabel}</span></div><ol>{score.score.components.map((component) => <li key={component.key}><span>{component.key.replaceAll("_", " ")}</span><strong>{component.points > 0 ? "+" : ""}{component.points}</strong><small>{component.explanation}</small></li>)}</ol></>}</section>;
}
