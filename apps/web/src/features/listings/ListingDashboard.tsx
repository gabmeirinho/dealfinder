...
function ListingInspector({ listing, client, onChange, onClose, onError }: {
  ...
  return (
    <article className="listing-inspector" aria-labelledby="listing-detail-title">
      ...
      <section className="fact-comparison"><header><h3>Vehicle facts</h3><button className="text-action" type="button" onClick={() => setCorrectionOpen((open) => !open)}>{correctionOpen ? "Close correction" : "Correct a fact"}</button></header>{mileageSources?.conflict ? <aside className="listing-risk"><strong>Mileage conflict</strong><span>{sourceLabel(listing.source)} structured data says {formatMileage(mileageSources.structuredKm)}; {mileageSources.descriptionKm === null ? "the result card" : "the seller description"} says {formatMileage(mileageSources.descriptionKm ?? mileageSources.cardKm)}. The structured value is selected, but verify it.</span></aside> : null}<FactTable original={listing.normalizedFacts} effective={listing.effectiveFacts} corrections={new Set(listing.corrections.map(({ field }) => field))} />{mileageSources?.source === "facebook_structured" && !mileageSources.conflict ? <p className="muted-copy">Mileage selected from Facebook structured listing data.</p> : null}{correctionOpen ? <CorrectionForm listing={listing} client={client} onChange={onChange} onError={onError} /> : null}</section>

      <section className="original-copy"><h3>{listing.source === "standvirtual" ? "Captured description" : "Original listing text"}</h3><h4>{listing.original.title}</h4>{listing.original.description === null ? <><p className="muted-copy">{listing.source === "facebook" ? (listing.detailFacts === null ? "No description or Facebook vehicle metadata has been captured." : "No seller description was available, but Facebook vehicle metadata was captured.") : listing.detailFacts ? "No description was available in the captured details." : "No description was available in the result card."}</p>{listing.source === "facebook" && listing.detailFacts === null ? <button className="secondary-action" type="button" disabled={busy} onClick={() => void mutate(() => client.captureDescription(listing.id))}>Capture Facebook details</button> : null}</> : <p>{listing.original.description}</p>}{listing.source === "facebook" ? <ul>{listing.original.cardFacts.map((fact, index) => <li key={`${fact}-${index}`}>{fact}</li>)}</ul> : null}{listing.sourceUrl !== null ? <a href={listing.sourceUrl} target="_blank" rel="noopener noreferrer">Open {sourceLabel(listing.source)} listing</a> : <p className="muted-copy">The source URL was blocked because it did not match the expected secure marketplace URL.</p>}</section>

      {listing.source === "standvirtual" ? <StandvirtualDetails listing={listing} busy={busy} onCapture={() => void mutate(async () => {
        try { return await client.captureDescription(listing.id); }
        catch (error) { onChange(await client.get(listing.id)); throw error; }
      })} /> : null}
