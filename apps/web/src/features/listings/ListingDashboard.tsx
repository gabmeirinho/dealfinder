...
function ListingInspector({ listing, client, onChange, onClose, onError }: {
  ...
  return (
    <article className="listing-inspector" aria-labelledby="listing-detail-title">
      ...
      <section className="original-copy"><h3>Original listing text</h3><h4>{listing.original.title}</h4>{listing.original.description === null ? <><p className="muted-copy">No description was captured from the result card.</p><button className="secondary-action" type="button" disabled={busy} onClick={() => void mutate(() => client.captureDescription(listing.id))}>Capture full description</button></> : <p>{listing.original.description}</p>}<ul>{listing.original.cardFacts.map((fact, index) => <li key={`${fact}-${index}`}>{fact}</li>)}</ul>{listing.sourceUrl !== null ? <a href={listing.sourceUrl} target="_blank" rel="noopener noreferrer">Open source listing</a> : <p className="muted-copy">Source URL was blocked because it was not a safe Facebook HTTPS URL.</p>}</section>
